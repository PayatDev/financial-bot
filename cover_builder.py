...
1. สถานการณ์ลูกค้า (2-4 ประโยค):
   ...
   - ประเด็น 03: ให้รวม 2 ย่อหน้า — (1) อายุลูกค้า อาชีพ รายได้ต่อเดือน ปีเกษียณ (60 ปี) เวลาทำงานที่เหลือ คำนวณ HLV = ใช้สูตร(TVM, PV=?, FV=0, I=4, N=60-อายุลูกค้า, PMT=รายได้ต่อเดือน×12) ครอบครัวพึ่งลูกค้า 100% หรือมีรายได้ทางอื่นชดเชยได้บ้างไหม (2) สรุปว่าจำเป็นต้องมีเครื่องมือทดแทนรายได้
...
2. คำแนะนำ (2-4 bullet ขึ้นต้นด้วย "- "):
   ...
   - ประเด็น 03: ให้ gen 3 bullet ตาม logic นี้เสมอ
     bullet 1 — คำนวณทุนประกันที่ต้องการ: หนี้สิน (debt) + เงินปรับตัว (income×70%×12) + HLV - ทรัพย์สินสุทธิ (cash+investment+savings ไม่รวมบ้านที่อยู่อาศัย) = ทุนที่ต้องการเพิ่ม ระบุตัวเลขจริงทุกขั้น ถ้ามีประกันอยู่แล้วให้หักด้วย แนะนำสำคัญให้ทำประกันชีวิตทุนประกันขั้นต่ำ 50% ของทุนที่ต้องการเพิ่ม
   ...
   - ประเด็น 09:
     ...
     bullet 3 — พูดคุยให้ผู้ดูแลเงินเข้าใจว่ามีหน้าที่ตามกฎหมายในการบริหารทรัพย์สินเพื่อประโยชน์ของเด็กเท่านั้น ไม่ใช่ของตัวเอง และต้องพร้อมรับผิดชอบหากมีการร้องเรียน
   - ประเด็น 10:
     ...
     bullets — ส่งกลับ [] เพราะ fixed_bullets ใน ISSUE_FIXED[10] จะจัดการให้เอง
   - ประเด็น 11:
     situation — ครั้งสุดท้ายที่ลูกค้า review แผนนี้เมื่อไหร่ ชีวิตเปลี่ยนอะไรบ้างตั้งแต่ทำแผน (ลูกเพิ่มขึ้น รายได้เปลี่ยน อสังหาริมทรัพย์เพิ่ม) สิ่งที่ต้อง update ในพินัยกรรมคืออะไรบ้าง
     situation — ลูกค้าอายุเท่าไหร่ตอนทำแผนนี้ คู่สมรสอายุเท่าไหร่ ลูกแต่ละคนอายุเท่าไหร่ ส่วนที่ทำได้ทันทีแล้วมีอะไรบ้าง และสิ่งที่ยังยากที่สุดคือประกันชีวิต โรคร้าย และทุพพลภาพที่ยังต้องติดตามต่อ
     bullet 1 — จัดทำตามแผนนี้ให้เสร็จตามลำดับ แม้ยังไม่ครบทุกอย่าง ดีกว่าไม่มีเลย แล้วค่อยๆ ปรับไปเรื่อยๆ ตั้งเตือนใน calendar ทบทวนปีละครั้ง
     bullet 2 — ติดตามประกันชีวิต โรคร้าย และทุพพลภาพ เป็นสิ่งที่ยังค้างอยู่ ให้ค่อยๆ ดำเนินการให้ครบตามความสามารถ
     bullet 3 — ทุกครั้งที่ชีวิตเปลี่ยน ให้ปรับเปลี่ยนแผนและคุยกับผู้เกี่ยวข้องให้รู้เรื่อง ทั้งผู้จัดการมรดก ผู้ปกครอง และคู่สมรส
   - ประเด็น 12:
     situation — ระบุว่าลูกค้ายังไม่ได้เขียนจดหมายถึงคู่สมรส (ชื่อ) และลูกแต่ละคน (ชื่อ) แยกเป็นรายบุคคล ขึ้นต้นด้วย "ปัจจุบัน คุณ[ชื่อ]ยังไม่ได้เขียนจดหมายถึง[ชื่อคู่สมรส]..."
     bullets — ส่งกลับ [] เสมอ เพราะ fixed_bullets ใน ISSUE_FIXED[12] จะจัดการให้เอง
   - ถ้าประเด็น 07, 10, 12 ให้ส่งกลับ [] (list ว่าง)
