...
10. ใส่ "คุณ" นำหน้าชื่อลูกค้าเสมอ
11. placeholder ใช้รูปแบบ [ข้อความอธิบาย] เสมอ
...
def build_will_docx(client_data: dict, clause3_items: list) -> str:
    ...
    _para(doc,
          f"ข้าพเจ้าขอแต่งตั้งให้ [ชื่อ-นามสกุลจริง{executor}] "
          f"ถือบัตรประจำตัวประชาชนเลขที่ [เลขบัตรประชาชน{executor}] "
    ...
    spouse = d.get("คู่สมรส", "[คู่สมรส]")
    _clause(doc, "๔", "แผนสำรอง")
    _para(doc,
          f"หากคู่สมรสของข้าพเจ้า [ชื่อ-นามสกุลจริงคุณ{spouse}] "
    ...
    gp  = d.get("Guardian หลัก",        "[ผู้ปกครองหลัก]")
    gb  = d.get("Guardian สำรอง",       "[ผู้ปกครองสำรอง]")
    mgp = d.get("Money Guardian หลัก",  "[ผู้ดูแลทรัพย์สินหลัก]")
    mgb = d.get("Money Guardian สำรอง", "[ผู้ดูแลทรัพย์สินสำรอง]")
    ...
    _para(doc, "มีหน้าที่ดูแลการเลี้ยงดู การศึกษา และความเป็นอยู่ของบุตร",
          space_after=4)
    _para(doc,
          f"หลัก: [ชื่อ-นามสกุลจริง{gp}] "
          f"ถือบัตรประจำตัวประชาชนเลขที่ [เลขบัตรประชาชน{gp}] "
          "ซึ่งเป็น [ความสัมพันธ์กับบุตร]")
    _para(doc,
          f"สำรอง: [ชื่อ-นามสกุลจริง{gb}] "
          f"ถือบัตรประจำตัวประชาชนเลขที่ [เลขบัตรประชาชน{gb}]")
    _subhead(doc, "ผู้ดูแลทรัพย์สินของบุตร")
    _para(doc,
          "มีหน้าที่บริหารและควบคุมทรัพย์สินของบุตรให้เป็นไปตามเจตนารมณ์ของพินัยกรรมฉบับนี้",
          space_after=4)
    _para(doc,
          f"หลัก: [ชื่อ-นามสกุลจริง{mgp}] "
          f"ถือบัตรประจำตัวประชาชนเลขที่ [เลขบัตรประชาชน{mgp}] "
          f"ซึ่งเป็น [ความสัมพันธ์กับคุณ{n}]")
    _para(doc,
          f"สำรอง: [ชื่อ-นามสกุลจริง{mgb}] "
          f"ถือบัตรประจำตัวประชาชนเลขที่ [เลขบัตรประชาชน{mgb}]")
    _para(doc,
    ...
    _sig_line(doc, "ผู้ทำพินัยกรรม", f"ชื่อ-นามสกุลจริงคุณ{n}")
    _spacer(doc, 10)
    _para(doc,
